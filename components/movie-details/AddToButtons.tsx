import React, { useContext } from "react";
import AddButton from "./AddButton";
import { MovieDetailsCtx } from "@/utils/movie-details-ctx";
import { MovieObj } from "@/pages/movie/[movieId]";

const AddToButtons = () => {
  const movieCtx: MovieObj = useContext(MovieDetailsCtx).movieData;
  async function BtnHandler(button: string) {
    console.log(movieCtx.id);

    const res = await fetch("/api/addMoviess", {
      method: "POST",
      body: JSON.stringify({
        button,
        movieId: "0",
      }),
      headers: {
        "Content-Type": "application/json",
      },
    });
  }
  const buttonsData = [
    {
      title: "Favorites",
      subTitle: "Add to",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="black"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="white"
          className="w-14 h-14"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
          />
        </svg>
      ),
      contStyle: "bg-white hover:bg-gray-200",
      textStyle: "text-black",
      clickHandler: () => BtnHandler("fav"),
    },
    {
      title: "Wishlist",
      subTitle: "Add to",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="white"
          className="w-10 h-10 "
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M21 11.25v8.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 1 0 9.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1 1 14.625 7.5H12m0 0V21m-8.625-9.75h18c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125h-18c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z"
          />
        </svg>
      ),
      contStyle: "border-2 border-white hover:border-gray-400",
      clickHandler: () => BtnHandler("wishlist"),
    },
    {
      title: "Collection",
      subTitle: "Save to",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="w-8 h-8"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z"
          />
        </svg>
      ),
      textStyle: "hover:text-gray-400",
      clickHandler: () => console.log("c clicked"),
    },
  ];

  return (
    <div className="flex gap-x-4 ">
      {buttonsData.map((item) => (
        <AddButton {...item} />
      ))}
    </div>
  );
};

export default AddToButtons;
